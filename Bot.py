...
async def 도움말(ctx):
    ...
    embed.add_field(name = "Command", value = "사용 설명서: (필수) =join 한 다음 인정-music 채널 가서 제목만 적으세요 \n        딜레이 심하니까 될때까지 기다렸다가 입력하세요 안그럼 오류남 ( 오류나면 =stop 하고 다시하면 돼요 ) \n        다른 채널이나 채널 안들어가 있어도 노래 틀 수 있어요 \n/join /leave /play (노래제목) /n (검색어) /g (검색어) \n/queuedel (숫자) /queue /queueclear \n/musicinfo /pause /resume /skip /stop \n/musicchannel /music_ch_video /music_ch_queue", inline = True)
